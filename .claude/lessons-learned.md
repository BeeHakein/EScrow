...
### 2026-06-20 — Prisma DB adapters (in-memory repos → real persistence)
- [architecture] All 6 repository ports got Prisma adapters in `src/infrastructure/db/prisma/` with ZERO caller change — same port-swap as the hashers/analyzer/parser. In-memory repos kept for fast application unit tests; the Prisma adapters carry their own round-trip tests. PrismaClient injected via constructor (inject-the-dependency) so tests pass a temp-sqlite client; `prisma/client.ts` is the app singleton.
- [architecture] Persistence is OUTSIDE the trust boundary like the AI/network: added `src/boundary/db-row.ts` that RE-PARSES every row back into domain types (reusing the existing value-object parsers) so Prisma types never leak into domain/application. It returns Result; the repo unwraps with `orThrow` because a row that won't parse is corruption (exceptional), unlike "not found" which is `null`.
- [architecture] Hybrid mapping, not all-or-nothing: NORMALIZE flat one-to-many lists as relations (Contract→Clause, RiskReport→ClauseRisk, replaced wholesale on save via nested `deleteMany`+`create`); SERIALIZE the heterogeneous discriminated unions (EscrowState 7 variants, MilestoneStatus 6) as canonical JSON in a TEXT column. Storing the unions as flat nullable columns would reintroduce the exact nullable-soup the domain union is designed to forbid — serialize + re-parse keeps the typed union the source of truth.
- [tooling] Prisma `Json` type is NOT supported on sqlite — that's why the unions are TEXT (manual JSON.stringify/parse), which also keeps models provider-portable. Same reason Wei is a base-10 TEXT string (no float, and avoids bigint-on-sqlite surprises) and Timestamp is ISO TEXT (canonical ISO sorts chronologically, so `findByContract` latest-by-`generatedAt` is a plain `orderBy desc`).
- [tooling] One committed sqlite schema (not a separate prod/test schema) — cross-provider generated clients are bound to their engine, so dual-provider is real complexity. Models are written provider-portable; prod swap = flip datasource `provider` to postgresql + re-migrate, models unchanged. The prime directive is a REAL offline/deterministic gate, and sqlite delivers that.
- [testing] Offline gate without a DB server: each test file spins up a THROWAWAY temp sqlite file (`mkdtemp`) and applies the COMMITTED migration via `prisma migrate deploy` (`tests/infrastructure/prisma/test-db.ts`), then injects a `new PrismaClient({ datasourceUrl })`. Parallel test files never contend (unique temp files). Exercises the real migration, not an ad-hoc schema. +37 tests (62→99).
- [types] `noUncheckedIndexedAccess` widens dot-access on `Record<string, T>` to `T | undefined`. For a fixtures lookup table use `const x = {...} satisfies Record<string, T>` (NOT `: Record<…>`) so the literal keys survive and `x.active` stays `T`.
- [testing] Clause id is a GLOBAL primary key (not scoped per contract): a fixture that reused `clause-0` across two contracts hit a unique-constraint failure on the nested create (Prisma attributed it to the outer `contract.upsert`, misleadingly). Real uploads mint unique clause ids; fixtures must too.
- [process] Gate green BOTH halves: typecheck clean + 99 vitest AND 13 forge. Prereq after fresh install: `npx prisma generate` (downloads engine once; network needed once). New runtime dep `@prisma/client` (+ `prisma` dev). On branch `feat/prisma-adapters` stacked on `feat/real-adapters` (PR #1 not yet merged).
